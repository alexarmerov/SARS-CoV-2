...
# This script identifies the genomic positions of minor variants that lead to synonymous amino acid substitutions.

     my $file = $ARGV[0] ;
     my $genomic_position = $ARGV[1] ;

# Parameters:
# file: minority synonymous codons
# genomic_position: initial position of the gene in the genome

     open ( my $fl , '<', $file ) or die "Can't read file $file" ;

  my %Mat ;

# Identify the genomic positions and the nucleotides that differentiate the minor codon from the reference codon.

 print join("\t", "aminoacid_position", "reference_codon", "codon", "aminoacid_reference"  ,  "aminoacid" , "nucleotide", "genomic_position", "number_reads_codon", "number_reads_position", "codon_frequency" ),"\n";
